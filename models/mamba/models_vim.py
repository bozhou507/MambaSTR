...
from .bimamba_simple import BiMamba, Block
...
def create_block(
    ...
    mixer_cls = partial(BiMamba, layer_idx=layer_idx, bimamba_type=bimamba_type, if_devide_out=if_devide_out, init_layer_scale=init_layer_scale, **ssm_cfg, **factory_kwargs)
    norm_cls = partial(
...
class VisionMamba(nn.Module):
    def __init__(self, 
        ...
        if if_rope:
            if if_cls_token:
                assert not self.use_double_cls_token and not self.use_middle_cls_token
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False, already_patch_embed=False):
    ...
    def forward(self, x, return_features=False, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False, already_patch_embed=False):
        x = self.forward_features(x, inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank, already_patch_embed=already_patch_embed)
