...
try:
    ...
    import warnings
    warnings.warn('causal_conv1d_fn, causal_conv1d_update = None, None')
...
try:
    ...
    import warnings
    warnings.warn('selective_state_update = None')
...
class Mamba(nn.Module):
    ...
    def count_extra_flops(m, args, output):
        ...
        # 2 MambaInnerFn
        # 2.1 causual conv1d
        m.total_ops += (L + m.d_conv - 1) * m.d_inner * m.d_conv
        # 2.2 x_proj
        m.total_ops += L * m.x_proj.in_features * m.x_proj.out_features
        # 2.3 dt_proj
        m.total_ops += L * m.dt_proj.in_features * m.dt_proj.out_features
        # 2.4 selective scan
        """
        u: r(B D L)
        delta: r(B D L)
    ...
    def step(self, hidden_states, conv_state, ssm_state, da_mask=1.0):
        ...
        if selective_state_update is None:
            ...
            y = selective_state_update(
                ssm_state, x, dt, A, B, C, self.D, z=z, dt_bias=self.dt_proj.bias, dt_softplus=True, da_mask=da_mask
            )
...
class Block(nn.Module):
    def __init__(
        self, dim, mixer_cls, mlp_cls, layer_idx=None, norm_cls=nn.LayerNorm, fused_add_norm=False, residual_in_fp32=False
    ):
        """
        ...
        self.mixer = mixer_cls(dim, layer_idx=layer_idx)
...
class MambaBlock(Block):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        mixer_cls=partial(Mamba, **kwargs)
        norm_cls = partial(nn.LayerNorm if RMSNorm is None else RMSNorm, eps=eps)
        if with_mlp:
            mlp_cls = partial(PositionwiseFeedForward, hidden=int(dim * mlp_hidden_ratio))
        else:
            mlp_cls = None
        super().__init__(
            dim,
