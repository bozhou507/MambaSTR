...
def get_window_kernel(K: str, H: int, W: int):
    if K in ['s', 'l']:
        hw = 2 if H % 2 == 0 else 3
        ww = 2 if W % 2 == 0 else 3
        if K == 'l':
            if H // hw > hw:
                hw = H // hw
            if W // ww > ww:
...
def local_scan(x, K=7, H=14, W=14, flip=False, column_first=False, allow_padding=True):
    ...
    Return: [B, C, L]
    """
    hw, ww = get_window_kernel(K, H, W)
    B, L, C = x.shape
    x = x.view(B, H, W, C)
    Hg, Wg = math.ceil(H / hw), math.ceil(W / ww)
    if H % hw != 0 or W % ww != 0:
        assert allow_padding is True, f'got (hw, ww) == {(hw, ww)}, (H, W) == {(H, W)}, but padding is not allowed'
        newH, newW = Hg * hw, Wg * ww
        x = F.pad(x, (0, 0, 0, newW - W, 0, newH - H))
    if column_first:
        x = x.view(B, Hg, hw, Wg, ww, C).permute(0, 5, 3, 1, 4, 2).reshape(B, C, -1)
    else:
        x = x.view(B, Hg, hw, Wg, ww, C).permute(0, 5, 1, 3, 2, 4).reshape(B, C, -1)
    if flip:
        x = x.flip([-1])
    return x


def local_scan_bchw(x, K=7, H=14, W=14, flip=False, column_first=False, allow_padding=True):
    """Local windowed scan in LocalMamba
    Input: 
        x: [B, C, H, W]
        H, W: original width and height before padding
    ...
    if H % hw != 0 or W % ww != 0:
        ...
        x = F.pad(x, (0, newW - W, 0, newH - H))
    if column_first:
        x = x.view(B, C, Hg, hw, Wg, ww).permute(0, 1, 4, 2, 5, 3).reshape(B, C, -1)
    else:
        x = x.view(B, C, Hg, hw, Wg, ww).permute(0, 1, 2, 4, 3, 5).reshape(B, C, -1)
    if flip:
        x = x.flip([-1])
    return x


def local_reverse(x, K=7, H=14, W=14, flip=False, column_first=False):
    """Local windowed scan in LocalMamba
    Input: 
        x: [B, C, L]
        H, W: original width and height before padding
    ...
    if H % hw != 0 or W % ww != 0:
        if column_first:
            x = x.view(B, C, Wg, Hg, ww, hw).permute(0, 1, 3, 5, 2, 4).reshape(B, C, Hg * hw, Wg * ww)
        else:
            x = x.view(B, C, Hg, Wg, hw, ww).permute(0, 1, 2, 4, 3, 5).reshape(B, C, Hg * hw, Wg * ww)
        x = x[:, :, :H, :W].reshape(B, C, -1)
    else:
        if column_first:
            x = x.view(B, C, Wg, Hg, ww, hw).permute(0, 1, 3, 5, 2, 4).reshape(B, C, L)
        else:
            x = x.view(B, C, Hg, Wg, hw, ww).permute(0, 1, 2, 4, 3, 5).reshape(B, C, L)
    return x
