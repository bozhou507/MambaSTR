...
from ..multi_mamba.multi_mamba import MultiMamba, Block
...
def create_block(
    ...
    token_size=(14, 14),
    mamba_cls=None,
    mamba_cls_kwargs=dict(),
):
    if ssm_cfg is None:
        ssm_cfg = {}
    factory_kwargs = {"device": device, "dtype": dtype}
    mixer_cls = partial(mamba_cls, layer_idx=layer_idx, bimamba_type=bimamba_type, directions=directions, token_size=token_size, use_middle_cls_token=use_middle_cls_token, **ssm_cfg, **factory_kwargs, **mamba_cls_kwargs)
    norm_cls = partial(
...
class VisionMamba(nn.Module):
    def __init__(self, 
                 ...
                 mamba_cls_kwargs: dict=dict(with_moe=False),
        ...
        self.token_size = self.patch_embed.grid_size if fixed_token_size else None
        ...
        if if_rope:
            ...
            if isinstance(img_size, (tuple, list)):
                hw_seq_len = img_size[0] // ps
            else:
                hw_seq_len = img_size // ps
        ...
        self.layers = nn.ModuleList(
            [
                create_block(
                    ...
                    mamba_cls=mamba_cls,
                    mamba_cls_kwargs=mamba_cls_kwargs,
                    **factory_kwargs,
                )
    ...
    def forward_features(self, x, token_sizes=None, inference_params=None, out_indices=None, already_patch_embed=False):
        # taken from https://github.com/rwightman/pytorch-image-models/blob/master/timm/models/vision_transformer.py
        # with slight modifications to add the dist_token
        if self.token_size is None:
            assert token_sizes is not None
        else:
            assert token_sizes is None
        if not already_patch_embed and self.patch_embed is not None:
            B, _, H, W = x.shape
        ...
        if self.if_abs_pos_embed:
            assert self.token_size is not None
            H, W = math.ceil(H / self.patch_size[0]), math.ceil(W / self.patch_size[1])
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            hidden_states, residual = layer(
                hidden_states, residual, token_sizes, inference_params=inference_params
            )
    ...
    def forward(self, x, token_sizes=None, return_features=False, inference_params=None):
        x = self.forward_features(x, token_sizes, inference_params=inference_params)
        if return_features:
            return x
        x = self.head(x)
        return x
...
def local_vim_tiny(pretrained=False, **kwargs):
    directions = (
        ['h', 'v_flip', 'wl', 'wl_flip'],
        ['h_flip', 'ws_flip', 'wl', 'wl_flip'],
        ['h', 'h_flip', 'v', 'wl'],
        ['h', 'h_flip', 'v', 'v_flip'],
        ['h', 'h_flip', 'v', 'v_flip'],
        ['h', 'h_flip', 'v', 'v_flip'],
        ['h_flip', 'v', 'v_flip', 'wl'],
        ['h_flip', 'v', 'v_flip', 'ws_flip'],
        ['h', 'h_flip', 'v', 'v_flip'],
        ['h', 'h_flip', 'v', 'v_flip'],
        ['h', 'v', 'v_flip', 'ws'],
        ['h', 'v', 'v_flip', 'ws_flip'],
    ...
    default_kwargs = dict(
        ...
        if_rope=True, if_rope_residual=True, if_abs_pos_embed=True, bimamba_type="v2", directions=directions, mamba_cls=MultiMamba
    )
    default_kwargs.update(kwargs)
    model = VisionMamba(**default_kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
...
def local_vim_tiny_wo_search(pretrained=False, **kwargs):
    directions = (('h', 'h_flip', 'ws', 'ws_flip'),) * 20
    model = VisionMamba(
        patch_size=16, embed_dim=192, depth=20, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', 
